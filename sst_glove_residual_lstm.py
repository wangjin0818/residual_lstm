...
def identity_block(input_tensor, hidden_dim, layer_id, dropout=0.25):
    lstm_name_base = 'res' + str(layer_id) + '_branch'

    x = LSTM(hidden_dim, return_sequences=True, name=lstm_name_base + '2a') (input_tensor)
    x = Dropout(dropout) (x)

    x = LSTM(hidden_dim, return_sequences=True, name=lstm_name_base + '2b') (x)
    x = Dropout(dropout) (x)
...
if __name__ == '__main__':
    ...
    logging.info("n_train_sample [n_train_sample]: (%d, %d)" % X_train.shape)

    n_test_sample = X_test.shape[0]
    logging.info("n_test_sample [n_test_sample]: (%d, %d)" % X_test.shape)
    ...
    x = identity_block(embedded, hidden_dim, '1a')
    x = identity_block(embedded, hidden_dim, '1b')
    x = identity_block(embedded, hidden_dim, '1c')
    x = identity_block(embedded, hidden_dim, '1d')
    x = identity_block(embedded, hidden_dim, '1e')

    lstm = LSTM(hidden_dim) (x)
